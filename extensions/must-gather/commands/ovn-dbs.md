...
7. **Run custom OVSDB query** (Query Mode):
   ...
   ```
   Query ACLs with priority > 1000 across all nodes. Gemini can construct the JSON query for any OVSDB table.
...
9. **Query specific table** (Gemini constructs JSON):
   Just ask Gemini to query a specific OVSDB table and it will construct the appropriate JSON query. For example:
...
- **Gemini Query Construction**: Gemini can automatically construct OVSDB JSON queries based on natural language requests
...
- **--query, -q** (json-query): Optional. Run a raw OVSDB JSON query instead of standard analysis. Gemini can construct the JSON query based on OVSDB transaction format. When provided, outputs raw JSON results instead of formatted analysis.
