...
**All replies must include**: `---\n*AI-assisted response via Gemini CLI*`
...
```bash
CHECK_REPLIED="${GEMINI_EXTENSION_ROOT}/scripts/check_replied.py"
if [ ! -f "$CHECK_REPLIED" ]; then
  CHECK_REPLIED=$(find ~/.gemini/extensions -type f -path "*/utils/scripts/check_replied.py" 2>/dev/null | sort | head -1)
fi
