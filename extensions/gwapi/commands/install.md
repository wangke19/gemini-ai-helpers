...
5. **Install GatewayClass**
   - Locate `extensions/gwapi/resources/gatewayclass.yaml`
   - Display: "Installing GatewayClass..."
   - Apply the resource: `oc apply -f extensions/gwapi/resources/gatewayclass.yaml` or `kubectl apply -f extensions/gwapi/resources/gatewayclass.yaml`
...
6. **Install Gateway with Domain Substitution**
   - Locate `extensions/gwapi/resources/gateway.yaml`
   - Display: "Installing Gateway with domain: $DOMAIN"
   - Export the domain as an environment variable: `export DOMAIN="<cluster-domain>"`
   - Substitute the domain in the YAML file using envsubst: `envsubst < extensions/gwapi/resources/gateway.yaml | oc apply -f -`
...
## Notes
- YAML files should be placed in `extensions/gwapi/resources/` directory:
