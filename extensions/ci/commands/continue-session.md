---
description: Download and continue a Gemini session from a Prow CI job's artifacts
...
If no sessions are found after extraction, tell the user "No Gemini sessions found in the extracted archives." and stop.
...
### Step 7: Copy selected session to local Gemini projects directory
...
```text
You are all set! You can now continue the session by launching a new Gemini:
