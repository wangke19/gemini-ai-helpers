...
```bash
${GEMINI_EXTENSION_ROOT}/commands/delete/list.sh
```
...
2. Run list again with the user's paths:
   ```bash
   ${GEMINI_EXTENSION_ROOT}/commands/delete/list.sh --repos-root <REPOS_PATH> --workspaces-root <WORKSPACES_PATH>
   ```
...
```bash
${GEMINI_EXTENSION_ROOT}/commands/delete/execute.sh {WORKSPACE_NAME}
```
...
```bash
${GEMINI_EXTENSION_ROOT}/commands/delete/execute.sh {WORKSPACE_NAME} --keep-branches
```

**If user chose "Delete branches"**:
```bash
${GEMINI_EXTENSION_ROOT}/commands/delete/execute.sh {WORKSPACE_NAME} --delete-branches
```
